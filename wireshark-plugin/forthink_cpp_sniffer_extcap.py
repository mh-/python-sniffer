...
def compile_cpp_capture():
    src_file = os.path.join(SCRIPT_DIR, "cpp_usb_capture.cpp")
    bin_file = os.path.join(SCRIPT_DIR, "cpp_usb_capture")
    
    if not os.path.exists(bin_file) or os.path.getmtime(src_file) > os.path.getmtime(bin_file):
        log_to_file("Compiling cpp_usb_capture.cpp...")
        import subprocess
        try:
            subprocess.run(["clang++", "-O3", "-std=c++11", src_file, "-o", bin_file], check=True)
            log_to_file("Compilation successful.")
        except subprocess.CalledProcessError as e:
            sys.stderr.write(f"Failed to compile cpp_usb_capture.cpp: {e}\n")
            sys.exit(1)
...
def list_interfaces():
    ...
    ORIGINAL_STDOUT.write("extcap {version=1.0.0}{display=Forthink C++ UWB Sniffer Extcap}\n")
    ...
    try:
        ...
        if not devices:
            ...
            ORIGINAL_STDOUT.write("interface {value=forthink_cpp_uwb_sniffer}{display=Forthink C++ UWB Sniffer}\n")
        else:
            for dev in devices:
                ORIGINAL_STDOUT.write("interface {value=forthink_cpp_uwb_sniffer}{display=Forthink C++ UWB Sniffer}\n")
    except Exception as e:
        sys.stderr.write(f"Error scanning devices: {e}\n")
        ORIGINAL_STDOUT.write("interface {value=forthink_cpp_uwb_sniffer}{display=Forthink C++ UWB Sniffer (Scan Error)}\n")
...
def capture(interface, fifo, channel, preamble_id, sfd_id):
    ...
    target_index = None
    if interface.startswith("forthink_cpp_uwb_sniffer_"):
        try:
    ...
    try:
        ...
        dongle.ft4222_device._send_cmd(f"START_CPP_CAPTURE {fifo} {channel} {int(dongle.ft4222_device.is_ncj29d5)} {pcap_pseudo_header.hex()} {bytes(start_cmd_bytes).hex()}")
